        ...
        private static void ProcessNextSceneInQueue(List<string> remainingScenes, string startScenePath, TransformChangesStore tStore, ComponentChangesStore cStore)
{
    ...
    else
    {
        ...
        RemoveChangesForSceneFromStore(currentPath, tStore, cStore);
        EditorApplication.delayCall += () => ProcessNextSceneInQueue(remainingScenes, startScenePath, tStore, cStore);
    }
}

        private static void RemoveChangesForSceneFromStore(string targetScenePath, TransformChangesStore tStore, ComponentChangesStore cStore)
        {
            ...
            if (tStore != null && tStore.changes.Count > 0)
            {
                var transformChangesToRemove = tStore.changes.FindAll(c => 
                    string.Equals(NormalizeScenePath(c.scenePath), targetScenePath, StringComparison.OrdinalIgnoreCase));
